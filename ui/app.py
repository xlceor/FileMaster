...
class FileCheckerApp(tk.Tk):
    def __init__(self):
        ...
        # Load Config & Initialize Variables
    ...
    def _init_vars(self):
        self.vars = {
            ...
            "status":       tk.StringVar(value="Sistema Listo"),
        }
    ...
    def _build_main_layout(self):
        # Sidebar
        self.sidebar = Sidebar(self, on_nav_change=self.navigate, width=240)
        ...
        self.header_title = ttk.Label(self.header, text="Configuración", 
    ...
    def navigate(self, target):
        ...
        if self.is_running and target != "results":
            # Allow seeing results while running, but maybe not config? 
            # Actually, let's allow navigation but notify.
            pass
        ...
        if target == "config":
            self.header_title.configure(text="Configuración del Sistema")
            ...
            self.header_title.configure(text="Panel de Resultados")
    ...
    def _handle_run(self):
        ...
        if mode in ["export", "compare"]:
            ...
            if not folder or not os.path.isdir(folder):
                messagebox.showerror("Error de Validación", "Por favor seleccione una carpeta de archivos válida.")
                return
        
        if mode == "compare_excel":
            source = self.vars["source_excel"].get().strip()
            if not source or not os.path.isfile(source):
                messagebox.showerror("Error de Validación", "Por favor seleccione un Excel de entrada válido.")
                return
        
        if mode in ["compare", "compare_excel"]:
            excel = self.vars["excel"].get().strip()
            if not excel or not os.path.isfile(excel):
                messagebox.showerror("Error de Validación", "Por favor seleccione un Excel maestro válido.")
                return
    ...
    def _task_processing(self):
        try:
            ...
            if mode == "export":
                ...
                self.after(0, lambda: self._on_task_complete(f"✔ Reporte generado: {out}", out))
            else:
    ...
    def _update_results_and_navigate(self, results):
        ...
        self.navigate("results")
        self._on_task_complete("✔ Comparación finalizada con éxito.")
    ...
    def _task_export(self):
        try:
            ...
            self.after(0, lambda: self._on_task_complete(f"✔ Reporte exportado: {out_path}", out_path))
        except Exception as e:
            self.after(0, lambda: self._on_task_error(str(e)))
    ...
    def _start_task(self, target):
        ...
        self.vars["status"].set("Procesando operación... por favor espere.")
    ...
    def _on_task_complete(self, msg, file_to_open=None):
        ...
        if file_to_open:
            if messagebox.askyesno("Operación Exitosa", f"{msg}\n\n¿Desea abrir el archivo ahora?"):
    ...
    def _on_task_error(self, error_msg):
        self.is_running = False
        self.progress.stop()
        self.vars["status"].set(f"✖ Error: {error_msg}")
        if "config" in self.views: self.views["config"].run_btn.state(['!disabled'])
        messagebox.showerror("Error en la Operación", f"Se produjo un error:\n{error_msg}")
